...
<div class="space-y-4">
  ...
  {#if $isLoading && !$searchResults}
    <div class="py-6 text-center text-base text-slate-500">Searching...</div>
  {:else if $searchResults}
    <div class="space-y-3">
      <div class="text-sm font-medium uppercase text-slate-500">Snipsels</div>
      {#if $searchResults.snipsels.length === 0}
        <div class="text-base text-slate-500">No snipsels</div>
      {:else}
        <div class="space-y-2">
          {#each $searchResults.snipsels as s}
            <button
              class="w-full rounded-md border bg-white px-4 py-3 text-left text-base hover:bg-slate-50"
              type="button"
              onclick={() => openSnipsel(s.id)}
            >
              <div class="flex items-center justify-between gap-2">
                <span class="font-medium">{s.type}</span>
                <span class="text-sm text-slate-500">{new Date(s.modified_at).toLocaleString()}</span>
              </div>
  ...
  {:else}
    <div class="text-base text-slate-500">Type a query in the header and press Enter.</div>
  {/if}
</div>
