...
<div class="min-h-screen bg-slate-50 text-slate-900">
  <header class="sticky top-0 z-10 border-b bg-white/95 backdrop-blur">
    <div class="mx-auto flex max-w-3xl items-center justify-between gap-3 px-4 py-3">
      ...
      <input
        class="min-w-0 flex-1 rounded-md border border-slate-200 bg-slate-100 px-3 py-2 text-sm"
        placeholder="Search"
        type="search"
  ...
  {#if $currentUser}
    <nav class="fixed bottom-0 left-0 right-0 border-t bg-white">
      <div class="mx-auto grid max-w-3xl grid-cols-5 gap-1 px-2 py-2 text-xs text-slate-600">
        <button
          class="rounded-md px-2 py-2 {$currentView.type === 'collections' ? 'bg-slate-100 font-medium' : ''}"
          type="button"
          onclick={openCollections}
        >
          Lists
        </button>
        <button class="rounded-md px-2 py-2" type="button" onclick={onNewSnipsel}>
          New
        </button>
        <button
          class="rounded-md px-2 py-2 {$currentView.type === 'calendar' ? 'bg-slate-100 font-medium' : ''}"
          type="button"
        ...
        <button
          class="rounded-md px-2 py-2 {$currentView.type === 'todos' ? 'bg-slate-100 font-medium' : ''}"
          type="button"
        ...
        <button
          class="rounded-md px-2 py-2 {$currentView.type === 'settings' ? 'bg-slate-100 font-medium' : ''}"
          type="button"
